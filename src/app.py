...
import re
import joblib
import cv2  # <--- 이게 먼저 있어야 MY_FONT를 쓸 수 있습니다.
import math
import time
import requests
...
# [스타일] 폰트 설정 (cv2 import 이후에 와야 함)
MY_FONT = cv2.FONT_HERSHEY_SIMPLEX 

# -----------------------------------------------------------------------------
# [경로 설정 수정] src 폴더 기준, 한 단계 상위(../) 폴더를 루트로 설정
# -----------------------------------------------------------------------------
# 1. 현재 파일(app.py)의 절대 경로 (예: .../src/app.py)
current_file_path = os.path.abspath(__file__)

# 2. 현재 파일이 있는 폴더 경로 (예: .../src)
current_dir = os.path.dirname(current_file_path)

# 3. [핵심] 한 단계 상위 폴더로 이동 (예: .../Virtual-Build-PC)
# src 폴더 밖에 데이터가 있으므로 부모 폴더로 올라갑니다.
...
# =========================================================
# 🚨 [디버깅] 경로 확인용 (성공하면 지우세요)
st.sidebar.markdown("### 🐞 경로 디버깅")
st.sidebar.text(f"Root: {PROJECT_ROOT}")
if os.path.exists(BASE_DATA_DIR):
    st.sidebar.success("✅ Dataset 폴더 연결됨")
else:
    st.sidebar.error(f"❌ Dataset 폴더 못찾음: {BASE_DATA_DIR}")
# =========================================================
